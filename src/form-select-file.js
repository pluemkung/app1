...
export default function FormSelectFile() {
    ...
    const onClickButton = () => {
        ...
        let maxSize = selectMaxSize.current.value // ดึงค่าจาก select ตัวที่สอง
        if (inputFile.current.files.length > maxNumFiles) { // เทียบกับจำนวนที่เลือก
        ...
        for (let f of inputFile.current.files) {
            if (f.size > maxSize * 1000) {
                alert(`ขนาดของเเต่ละไฟล์ต้องไม่เกิน ${maxSize} KB`) // เติมข้อความแจ้งเตือน
                return
            }
        } 
        alert('Files OK')
